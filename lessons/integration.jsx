  ...
  return (
    <div>
      ...
      <FastPath
        minutes={100}
        must={[
          { s: "หมวด 0 + 1 · ทำไมต้องมี + ∫ คือผลรวม", min: 10, why: "ทุกวิธีในบทนี้คือ “ซอยแล้วบวก” เข้าใจอันนี้แล้วที่เหลือเป็นแค่การเปลี่ยนรูปที่ซอย" },
          { s: "หมวด 2 · Trapezoidal", min: 15, why: "วิธีแรกและง่ายสุด — เป็นฐานของ Composite" },
          { s: "หมวด 3 · Composite Trapezoidal", min: 15, why: "สูตรที่ใช้จริงเกือบทุกข้อ · จำน้ำหนัก 1-2-2-…-2-1" },
          { s: "หมวด 4 · Simpson’s Rule", min: 15, why: "พาราโบลาแทนเส้นตรง — น้ำหนัก 1-4-1" },
          { s: "หมวด 5 · Composite Simpson", min: 20, why: "⭐ จุดที่พลาดกันมากสุด: n = จำนวนพาราโบลา ⇒ ช่องย่อย = 2n" },
          { s: "หมวด 📟 · fx-991CW", min: 15, why: "ครึ่งหนึ่งของคะแนนอยู่ที่กดเครื่องไม่พลาด" },
          { s: "หมวด ∑ · สรุปสูตร 4 วิธี", min: 10, why: "ปิดท้ายด้วยการเห็นทั้ง 4 สูตรเรียงกันในหน้าเดียว" },
        ]}
        skip={[
          { s: "หมวด 🎁 Romberg & Gauss-Legendre", why: "อาจารย์บอกเองว่าเกินขอบเขตปีนี้" },
          { s: "หมวด 📉 กราฟ error เทียบ n", why: "ช่วยให้เข้าใจลึกขึ้น แต่ไม่มีข้อสอบถามตรง ๆ" },
          { s: "หมวด 🏗️ ประยุกต์ + ✸ ข้อสอบจำลอง", why: "ยกไปทำในเฟส 2–3 ตามแผน อย่าเพิ่งทำตอนอ่านรอบแรก" },
        ]}
        check={[
          "เขียนสูตรทั้ง 4 วิธีจากความจำได้ (Trap / Comp Trap / Simpson / Comp Simpson)",
          "ตอบได้ว่า “Simpson n = 3” หมายถึงกี่ช่องย่อย (ตอบ: 6)",
          "รู้ว่าถ้าจำนวนช่องย่อยเป็นเลขคี่ Simpson 1/3 ล้วนใช้ไม่ได้ แล้วต้องทำยังไง",
        ]}
      />
      ...
      <Sect tag="0" title="ทำไมต้องมี Numerical Integration? — อุปมา “วัดที่ดินพ่อ”" read="must" min={10}>
      ...
      <Sect tag="1" title="∫ คือ “ผลรวม” — เชื่อมนิยามกับ Riemann sum" read="must" min={10}>
      ...
      {/* ═══════════════ 2 · Trapezoidal ═══════════════ */}
      <Sect tag="2" title="① Trapezoidal Rule — แทนเส้นโค้งด้วยเส้นตรง 1 เส้น" read="must" min={15}>
...
f(x₁)=f(2) = 2(8)−5(4)+3(2)+1 = 16−20+6+1 = 3`,
  ...
  = 50.00%   ← พลาดครึ่งหนึ่งเลย!` },
      ...
      <Sect tag="3" title="② Composite Trapezoidal — ซอยเป็นคางหมูหลายอัน" read="must" min={15}>
...
Σ :  h/2( f0 + 2f1 + 2f2 + 2f3 + f4 )
      ...
      {/* ═══════════════ 4 · Simpson ═══════════════ */}
      <Sect tag="4" title="③ Simpson’s Rule — แทนเส้นโค้งด้วย “พาราโบลา” 3 จุด" read="must" min={15}>
      ...
      {/* ═══════════════ 5 · Composite Simpson ═══════════════ */}
      <Sect tag="5" title="④ Composite Simpson’s Rule — พาราโบลาหลายอัน" read="must" min={20}>
...
Σ            :  h/3( f0 + 4f1 + 2f2 + 4f3 + f4 )
      ...
      <Sect tag="✍️" title="แบบฝึกหัด 1 · ทำเต็มทุกข้อ (มือ + เครื่องคิดเลข + โปรแกรม)" read="later" why="เป็นโจทย์ ยกไปเฟส 2">
      ...
      {/* ═══════════════ 🎮 · Interactive solver ═══════════════ */}
      <Sect tag="🎮" title="ลองเล่น · Integration Solver" read="later" why="ไว้ตรวจคำตอบทีหลัง">
        <p>พิมพ์ฟังก์ชันอะไรก็ได้ เลือกวิธี แล้วดูคำตอบทันที (ใช้ตรวจการบ้านได้)</p>
        <IntegrationSolver/>
      </Sect>

      {/* ═══════════════ 📉 · Error vs n ═══════════════ */}
      <Sect tag="📉" title="ทำไม Simpson แม่นกว่า — กราฟ error เทียบ n" read="later" why="ไม่ออกสอบ">
...
print(f"{'exact':<34}{exact:>12.6f}")`} height={310}/>
      ...
      {/* ═══════════════ ∑ · Quick Ref ═══════════════ */}
      <Sect tag="∑" title="สรุปสูตร · 4 วิธีหลัก (ที่ออกสอบปีนี้)" read="must" min={10}>
        <NumTable
      ...
      <Sect tag="🎁" title="ของแถม (เกินขอบเขตแบบฝึกหัดปีนี้) — Romberg & Gauss-Legendre" read="later" why="เกินขอบเขตปีนี้">
