...
function IntegrationLesson() {
  ...
  return (
    <div>
      ...
      <Sect tag="3.5" title="เขียนมือ · Composite Simpson 1/3 (สไตล์ข้อสอบ)">
        <p>คำนวณ <M>{`\\int_0^1 e^{x^2}\\, dx`}</M> ด้วย Composite Simpson 1/3, n = 4</p>
        <window.HandWalkthrough steps={[
          { title: "ตั้ง h + จุดข้อมูล",
            body: `h = (b − a) / n = (1 − 0) / 4 = 0.25
x₀ = 0,    x₁ = 0.25,  x₂ = 0.5,   x₃ = 0.75,  x₄ = 1.0` },
          { title: "คำนวณค่า f(xᵢ) = e^(xᵢ²)",
            body: `f(x₀) = e^(0²)    = e⁰     = 1.00000
f(x₁) = e^(0.0625) = 1.06449
f(x₂) = e^(0.25)   = 1.28403
f(x₃) = e^(0.5625) = 1.75505
f(x₄) = e^(1)      = 2.71828`,
            calc: "เก็บแต่ละค่าใน A, B, C, D, E:   |   e^(0.25²) → STO B  ฯลฯ" },
          { title: "ใช้สูตร Simpson 1/3 กฎ 1-4-2-4-1",
            body: `Pattern: f₀ + 4f₁ + 2f₂ + 4f₃ + f₄
       (สี่เหลี่ยมเปียก: คี่ × 4, คู่ × 2 ยกเว้นปลาย × 1)
...
S = 1.00000
  ...
  + 1(2.71828) = 2.71828
  รวม           = 17.56450`,
            calc: "1 + 4×B + 2×C + 4×D + E = → STO S" },
          { title: "คูณ h/3",
            body: `I ≈ (h/3) · S = (0.25/3) · 17.56450
   = 0.08333... · 17.56450
   = 1.46371

(ค่าจริง ≈ 1.46265 — error ~0.07%)`,
            calc: "0.25 ÷ 3 × S = → ได้คำตอบ" },
          { title: "ตรวจ + รายงาน",
            body: `ตรวจ: ทำ Composite Trap n=4 เทียบ:
T = h/2 · [f₀ + 2(f₁+f₂+f₃) + f₄]
  = 0.125 · [1 + 2(1.06449+1.28403+1.75505) + 2.71828]
  = 0.125 · [1 + 8.20714 + 2.71828]
  = 0.125 · 11.92542
  = 1.49068  (err ~1.9% — แย่กว่า Simpson 30 เท่า!)` },
        ]}/>
      </Sect>
