  ...
  return (
    <div>
      ...
      <FastPath
        minutes={105}
        must={[
          { s: "หมวด 0 + 1 · ความชัน + slope เฉลี่ย vs ชั่วขณะ", min: 10, why: "ปูพื้นว่าทำไมต้องประมาณอนุพันธ์แทนการ diff ตรง ๆ" },
          { s: "หมวด 3 · ย้ายข้าง Taylor → Forward + O(h)", min: 15, why: "⭐ เข้าใจว่า O(h) มาจากไหน แล้วจะเลิกท่อง 24 สูตรแบบจำมั่ว" },
          { s: "หมวด 4 · First Derivative 3 สาย", min: 20, why: "Forward / Backward / Central — โจทย์บังคับสายไหนต้องหยิบถูก" },
          { s: "หมวด 5 · Higher Derivatives", min: 15, why: "f″ ออกบ่อยพอ ๆ กับ f′ (การบ้าน 3 ข้อ 2 ก็ถาม f″)" },
          { s: "หมวด 6 · More Accurate O(h²)/O(h⁴)", min: 20, why: "⭐ โจทย์เขียน “O(h²)” = คำสั่งให้ใช้ชุดละเอียด หยิบผิดชุด = 0" },
          { s: "หมวด 📟 · fx-991CW", min: 10, why: "ตารางค่า f(x) หลายจุด กดผิดจุดเดียวพังทั้งข้อ" },
          { s: "หมวด ∑ · สรุป 24 สูตร + วิธีจำ", min: 15, why: "ไม่ต้องท่องทั้ง 24 — หมวดนี้บอกโครงที่ทำให้ไล่เอาเองได้" },
        ]}
        skip={[
          { s: "หมวด 2 · Taylor Series เต็ม ๆ", why: "อ่านแค่หมวด 3 ที่ย้ายข้างให้ดูก็พอสำหรับข้อสอบ" },
          { s: "หมวด 🎁 Richardson Extrapolation", why: "เกินขอบเขตปีนี้" },
          { s: "หมวด 📉 Error vs h (กราฟตัว U)", why: "น่าสนใจแต่ไม่ออกสอบ" },
          { s: "หมวด ✍️ 📮 🏗️ ✸", why: "เป็นโจทย์ — ยกไปเฟส 2–3" },
        ]}
        check={[
          "โจทย์บอก “forward O(h²)” แล้วหยิบสูตรถูกทันทีโดยไม่ต้องไล่ตาราง",
          "อธิบายได้ว่าทำไม central แม่นกว่า forward ทั้งที่ใช้จุดน้อยกว่า",
          "เขียนสูตร f″ central O(h²) จากความจำได้",
        ]}
      />
      ...
      {/* ═══════════════ 0 · ความชันคืออะไร ═══════════════ */}
      <Sect tag="0" title="ความชันคืออะไร? — อุปมา “เดินขึ้นเขา”" read="must" min={5}>
      ...
      <Sect tag="1" title="Slope เฉลี่ย vs ชั่วขณะ — ตัวอย่างเดินหลัก f(x) = 3x²" read="must" min={5}>
...
นี่แหละที่มาของ diff ที่ใช้ในวิชา Math` },
      ...
      {/* ═══════════════ 2 · Taylor series ═══════════════ */}
      <Sect tag="2" title="Taylor Series — “ซีรีส์ของนาย Taylor เข้าฉาย Netflix คืนนี้”" read="later" why="อ่านหมวด 3 แทนก็พอ">
      ...
      <Sect tag="3" title="ย้ายข้าง Taylor → สูตรแรก (Forward) + ความหมายของ O(h)" read="must" min={15}>
...
🎙️ "ตัวแรกที่มันเจอคือ h กำลังหนึ่ง" → error of order h` },
      ...
      {/* ═══════════════ 4 · 3 สาย ═══════════════ */}
      <Sect tag="4" title="First Derivative ครบ 3 สาย — Forward · Backward · Central" read="must" min={20}>
...
ε = |7.389056099 − 7.466266321| / 7.389056099 × 100
  = 1.04%   ← จาก ~12–13% เหลือ 1% ด้วยข้อมูลชุดเดิม`,
      ...
      <Sect tag="5" title="Higher Derivatives — f″, f‴, f⁗ (diff มากกว่า 1 ครั้ง)" read="must" min={15}>
...
อันดับ 4:  1  −4   6  −4   1        (แถว 1-4-6-4-1)`}</div>
      ...
      {/* ═══════════════ 6 · More accurate ═══════════════ */}
      <Sect tag="6" title="More Accurate Derivative — ชุด “ละเอียด” O(h²) / O(h⁴)" read="must" min={20}>
...
(ถ้าไม่ปัดกลางทาง โปรแกรมจะได้ 2.048288689 — ต่างเพราะปัด 9 ตำแหน่งในตาราง)` },
      ...
      {/* ═══════════════ 📟 · เครื่องคิดเลขไว ๆ ═══════════════ */}
      <Sect tag="📟" title="fx-991CW · สูตรลัดกดไว — ข้อสอบเยอะ เวลาน้อย" read="must" min={10}>
      ...
      {/* ═══════════════ ✍️ · แบบฝึกหัด 2 ═══════════════ */}
      <Sect tag="✍️" title="แบบฝึกหัด 2 · ทำเต็มทุกข้อ (มือ + เครื่องคิดเลข + โปรแกรม) — การบ้าน 6 คะแนน" read="later" why="เป็นโจทย์ ยกไปเฟส 2">
      ...
      {/* ═══════════════ 📮 · การบ้าน 3 ═══════════════ */}
      <Sect tag="📮" title="การบ้าน 3 · Diff + Graphical Method — เฉลยเต็ม + โปรแกรมส่งได้" read="later" why="เป็นโจทย์ ยกไปเฟส 2">
        <DiffHomeworkThree/>
      </Sect>

      {/* ═══════════════ 🎮 · Solver ═══════════════ */}
      <Sect tag="🎮" title="ลองเล่น · Differentiation Solver" read="later" why="ไว้ตรวจคำตอบทีหลัง">
        <p>พิมพ์ฟังก์ชันอะไรก็ได้ ใส่ x กับ h แล้วดูครบทุกสูตรพร้อมกัน (ใช้ตรวจการบ้านได้)</p>
        <DiffSolver/>
      </Sect>

      {/* ═══════════════ 📉 · U-shape ═══════════════ */}
      <Sect tag="📉" title="Error vs h — เส้นโค้งรูปตัว U ของ Floating Point" read="later" why="ไม่ออกสอบ">
      ...
      {/* ═══════════════ 🎁 · โบนัส ═══════════════ */}
      <Sect tag="🎁" title="ของแถม (เกินขอบเขตปีนี้) — Richardson Extrapolation" read="later" why="เกินขอบเขตปีนี้">
...
for name, v, ex in [("v O(h^2)", v_h2, v_ex), ("v O(h^4)", v_h4, v_ex),
    ...
    print(f"{name:<10} = {v:>10.6f}   exact = {ex:>9.6f}   err = {abs(ex-v)/abs(ex)*100:.6f}%")`} height={320}/>
      ...
      {/* ═══════════════ ∑ · Quick Ref ═══════════════ */}
      <Sect tag="∑" title="สรุป 24 สูตร + วิธีจำ (ที่ออกสอบปีนี้)" read="must" min={15}>
      ...
      {/* ═══════════════ ✸ · ข้อสอบจำลอง ═══════════════ */}
      <Sect tag="✸" title="ข้อสอบจำลอง — เขียนมือ + เขียนโปรแกรม (จับเวลาเลย)" read="later" why="เป็นโจทย์ ยกไปเฟส 3">
