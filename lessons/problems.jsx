...
const PROBLEMS = [
  ...
  { id: "R12", topic: "root", diff: "easy", title: "One-point Iteration ⭐ ตัวอย่างที่อาจารย์สอนในคาบ",
    q: <p>ใช้ One-point Iteration หาราก <M>{`f(x) = e^{-x} - x = 0`}</M> เริ่มที่ <M>{`x_0 = 0`}</M> จำนวน 4 รอบ พร้อม <M>{`\\varepsilon_a\\%`}</M> · และบอกว่าจัด <M>{`g(x)`}</M> ได้ยังไง</p>,
    a: <div>
      <p><b>จัดรูป:</b> แยก <M>{`f_1(x)=x`}</M> (เป็น x เสมอ) กับ <M>{`f_2(x)=e^{-x}`}</M> ⇒ <M>{`x = e^{-x}`}</M> ⇒ <M>{`x_{i+1}=e^{-x_i}`}</M></p>
      <StepTable headers={["i","xᵢ₋₁","xᵢ = e^(−xᵢ₋₁)","ε%"]} rows={[
        [1,0.0000000,1.0000000,"100.00"],
        [2,1.0000000,0.3678794,"171.83"],
        [3,0.3678794,0.6922006,"46.85"],
        [4,0.6922006,0.5004735,"38.31"],
      ]}/>
      <p><b>ตอบ</b> <M>{`x_4 = 0.5004735`}</M> (รากจริง 0.5671433) · <b>ค่าเด้งสลับสูง-ต่ำเป็นเรื่องปกติ</b> เพราะ <M>{`g'(x)=-e^{-x} < 0`}</M> และ <M>{`|g'|=0.5671<1`}</M> ⇒ ลู่เข้าแบบ linear · ต้องเดินถึงรอบ 14 จึงจะเข้าเกณฑ์ <M>{`|\\Delta x|<0.001`}</M></p>
    </div>
  },
  { id: "R13", topic: "root", diff: "medium", title: "One-point · เลือก g(x) ให้ลู่เข้า ⭐",
    q: <p><M>{`f(x) = x^2 - 7 = 0`}</M> จัดเป็น <M>{`x=g(x)`}</M> ได้หลายแบบ · จงบอกว่าแบบไหนใช้ได้ แล้วเดิน 4 รอบด้วยแบบที่ใช้ได้จาก <M>{`x_0 = 3`}</M></p>,
    a: <div>
      <p><b>เช็ค <M>{`|g'(\\sqrt7)| < 1`}</M> ก่อนเดินเสมอ</b> (<M>{`\\sqrt7 = 2.6457513`}</M>):</p>
      <StepTable headers={["g(x)","g′(√7)","ใช้ได้ไหม"]} rows={[
        ["7/x","−1.0000","❌ |g′| = 1 พอดี → วนกลับที่เดิมไม่จบ"],
        ["(7+x)/(x+1)","−0.4514","✅ ลู่เข้า"],
        ["(x + 7/x)/2","0","✅ เร็วมาก (= Newton ของ x²−7)"],
      ]}/>
      <p style={{marginTop:10}}><b>เดินด้วย</b> <M>{`g(x)=\\dfrac{7+x}{x+1}`}</M>:</p>
      <StepTable headers={["i","xᵢ₋₁","xᵢ","ε%"]} rows={[
        [1,3.0000000,2.5000000,"20.0000"],
        [2,2.5000000,2.7142857,"7.8947"],
        [3,2.7142857,2.6153846,"3.7815"],
        [4,2.6153846,2.6595745,"1.6615"],
      ]}/>
      <p><b>ตอบ</b> <M>{`x_4 = 2.6595745`}</M> (รากจริง <M>{`\\sqrt7 = 2.6457513`}</M>)</p>
      <p><b>ถ้าเผลอใช้ <M>{`g(x)=7/x`}</M>:</b> 3 → 2.3333333 → 3 → 2.3333333 → … <b>วนสองค่าไปตลอดไม่มีวันหยุด</b> — นี่คือสาเหตุที่ต้องเช็ค <M>{`|g'|`}</M> ก่อน ไม่ใช่เดินไปเรื่อย ๆ แล้วค่อยดู</p>
    </div>
  },
