...
## หน้า/คอมโพเนนต์ที่เพิ่มช่วงเตรียมมิดเทอม (ส.ค. 2569)
...
- `<FastPath minutes must skip check/>` — กล่องทางลัดหัวบท: อ่านหมวดไหน กี่นาที ข้ามอะไรได้
- `<Sect read="must|later" min why>` — ป้ายที่หัวข้อทุกหมวด 🟢 อ่านตอนนี้ / ⬜ ข้ามได้ (สไตล์ `.read-badge` ใน styles.css)
- `PythonRunner` โชว์กราฟ matplotlib ได้ (ดู § Python execution)

**อยู่ใน `lessons/midterm.jsx`:** `RandomExamDraw` (🎲 สุ่มชุดสอบ · `numer-lastdraw`) · `ErrorLog` (🩺 สมุดพลาด · `numer-errlog`)

**เมนูเรียงเป็นลำดับการใช้งาน** ไม่ใช่ลำดับบท: ① เริ่มที่นี่ → ② อ่าน 4 บท → ③ ท่อง → ④ ซ้อม → นอกขอบเขต midterm

## กฎเรื่องขอบเขตสอบ

ก่อนตัดสินใจว่าหัวข้อไหนออก/ไม่ออก **ถาม kim ก่อนว่าอาจารย์พูดว่าอะไร** — kim เคยเรียนวิชานี้มาแล้วและเข้าคาบที่ประกาศขอบเขต · ลำดับความน่าเชื่อ: คำอาจารย์ (kim จำได้/ไฟล์เสียง) > ใบงานปีนี้ > เอกสารติวปีที่แล้ว > สไลด์แผนที่คอร์ส · **เขียนขอบเขตลงแอปให้กำกับที่มาทุกบรรทัด** และถ้าเปลี่ยนคำแนะนำที่เคยให้ ต้องบอกตรง ๆ ว่าเคยแนะนำผิด
